...
padded_reviews = pad_sequence(encoded_reviews, batch_first=True, padding_value=pad_idx)



encoded_labels = [
    torch.tensor([vocab.get(tok, vocab["<unk>"]) for tok in label_text[l]], dtype = torch.long) for l in train_labels
]

class TransformerDataset(Dataset):
    def __init__(self, src_list, label_list,max_tgt_len=3):
        """
        src_list: list of encoded reviews (list of tensors)
        label_list: list of encoded labels (list of tensors)
        max_src_len: maximum source length (for truncation)
        max_tgt_len: maximum target length (for truncation, usually 3 for <sos> label <eos>)
        """
    ...
    def __getitem__(self, idx):
        # get source sequence and truncate if necessary
        src = self.src_list[idx]
        src_len = len(src)

        # get target sequence (label)
        tgt = self.label_list[idx][:self.max_tgt_len]
        tgt_len = len(tgt)

        # prepare decoder input (everything except last token)
        tgt_input = tgt[:-1]
        # prepare target output (everything except first token)
        tgt_output = tgt[1:]
...
# In Transformers (Hugging Face) and PyTorch, the collate_fn is a function used by the DataLoader to merge a list of dataset samples into a batch.
# When working with NLP models like Transformers, collate_fn is often used to pad sequences dynamically so that all inputs in a batch have the same length.
...
dataset = TransformerDataset(encoded_reviews, encoded_labels)
...
#     Residual_connection(dropout)    # index 1 ]
        ...
        self.residual_connections = nn.ModuleList(Residual_connection(nn.Dropout(dropout)) for _ in range(2))
...
class DecoderBlock(nn.Module):
    def __init__(self, self_attention_block, cross_attention_block, feed_forward_block, dropout):
        ...
        self.residual_connections = nn.ModuleList([
            Residual_connection(nn.Dropout(dropout)),
            Residual_connection(nn.Dropout(dropout)),
            Residual_connection(nn.Dropout(dropout))
        ])
...
def create_src_mask(src, pad_idx):
    ...
    # output: (batch, 1, 1, src_len)

def create_tgt_mask(tgt, pad_idx):
    batch, tgt_len = tgt.shape
    device = tgt.device  # use the tensor's actual device
    ...
    # Combine masks
    final_mask = pad_mask & causal_mask  # safe: all on same device

    return final_mask
#This combines two different masks into one final mask used by the Decoder.
...
def get_model(vocab_size):
    ...
    tgt_vocab = vocab_size  # same vocab

    model = build_project(
        src_vocab_size=src_vocab,
        trg_vocab_size=tgt_vocab,
        src_seq_len=200,
        trg_seq_len=3,   # label length is tiny
        d_model=48,
        N=2,
        h=4,
        dropout=0.1
...
def train_model():
    ...
    for epoch in range(10):
        ...
        for batch in dataloader:
            ...
            tgt_in = batch["tgt_input"].to(device)          # (batch, tgt_len-1)
            tgt_out = batch["tgt_output"].to(device)        # (batch, tgt_len-1)         
